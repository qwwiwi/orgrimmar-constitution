# Каналы принца

## Telegram-каналы

### 1. принц (основной)
- **Username:** канал принца
- **Ссылка:** https://t.me/канал-принца
- **Контент:** Криптоанализ, макро, личные проекты, инвестиционная философия
- **Память:** `memory/channels/channel-канал-принца.md`

### 2. DCA $10 каждый день
- **Ссылка:** https://t.me/+V-7YMMfVIt1jZWY6
- **Username:** уточнить у принца
- **Контент:** Ежедневная покупка крипты и акций на $10, трекинг портфеля
- **Память:** `memory/channels/channel-dca.md`

### 3. Записка AI энтузиаста
- **Username:** уточнить у принца
- **Ссылка:** https://t.me/+lVs9MrPaRoA5ODMy
- **Контент:** AI-инструменты, личный опыт использования, новости с комментарием
- **Память:** `memory/channels/channel-ai.md`

## YouTube

### 4. принц (YouTube)
- **URL:** https://www.youtube.com/канал принца
- **Контент:** Видео о крипте, AI, инвестициях, личный бренд
- **Память:** `memory/channels/channel-youtube.md`

## Контакты для связи
- **Telegram (личный):** канал принца, канал принца
- **Discord:** qwwiwi

## Проекты

---

_Username для DCA и AI каналов уточнить у принца при необходимости._
